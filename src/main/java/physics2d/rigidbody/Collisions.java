...
public class Collisions {
    ...
    public static CollisionManifold findCollisionFeatures(Circle a, Circle b){
        ...
        Vector2f distance = new Vector2f(b.getCentre()).sub(a.getCentre());
        if(distance.lengthSquared() - (sumRadii * sumRadii) > 0) {
            return result;
        }

        //Multiply by 0.5 because we want to separate each circle the same amount.Consider updating to factor in the momentum and velocity
        ...
        Vector2f contactPoint = new Vector2f(a.getCentre().add(new Vector2f(normal).mul(distanceToPoint)));
